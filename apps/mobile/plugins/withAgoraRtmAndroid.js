...
const LEGACY_DEPENDENCY_BLOCK = /\n?\/\/ @generated by withAgoraRtmAndroid\r?\ndependencies \{ implementation "io\.agora:agora-rtm:[^"]+" \}\r?\n?/g;
const LEGACY_PACKAGING_BLOCK = /\n?\/\/ @generated by withAgoraRtmAndroid libaosl packaging\r?\nandroid \{\r?\n    packagingOptions \{\r?\n        pickFirsts \+= \["\*\*\/libaosl\.so"\]\r?\n    \}\r?\n\}\r?\n?/g;

const GRADLE_START_MARKER = "// @generated by withAgoraRtmAndroid start";
const GRADLE_END_MARKER = "// @generated by withAgoraRtmAndroid end";

function createGradleBlock() {
  return `
${GRADLE_START_MARKER}
// RTC 4.6.x and RTM 2.2.x both bundle libaosl.so, but the RTM copy does not
// export symbols required by RTC (including aosl_ref_magic). Repackage RTM
// without its duplicate so the RTC-compatible libaosl.so is the only copy.
configurations {
    agoraRtmWithoutAosl
}

dependencies {
    agoraRtmWithoutAosl "io.agora:agora-rtm:${RTM_VERSION}"
}

def stripAgoraRtmAosl = tasks.register("stripAgoraRtmAosl", Zip) {
    archiveFileName = "agora-rtm-${RTM_VERSION}-without-aosl.aar"
    destinationDirectory = layout.buildDirectory.dir("generated/agora-rtm")
    from({ configurations.agoraRtmWithoutAosl.collect { zipTree(it) } }) {
        exclude "jni/**/libaosl.so"
    }
}

def strippedAgoraRtmAar = files(stripAgoraRtmAosl.flatMap { it.archiveFile })
strippedAgoraRtmAar.builtBy(stripAgoraRtmAosl)

dependencies {
    implementation strippedAgoraRtmAar
}
${GRADLE_END_MARKER}
`;
}

function replaceAgoraGradleBlock(source) {
  const generatedBlockPattern = new RegExp(
    `\\n?${GRADLE_START_MARKER}[\\s\\S]*?${GRADLE_END_MARKER}\\r?\\n?`,
    "g",
  );

  return source
    .replace(generatedBlockPattern, "\n")
    .replace(LEGACY_DEPENDENCY_BLOCK, "\n")
    .replace(LEGACY_PACKAGING_BLOCK, "\n")
    .trimEnd()
    .concat("\n", createGradleBlock());
}
...
function withAgoraRtmAndroid(config) {
  config = withAppBuildGradle(config, (nextConfig) => {
    nextConfig.modResults.contents = replaceAgoraGradleBlock(
