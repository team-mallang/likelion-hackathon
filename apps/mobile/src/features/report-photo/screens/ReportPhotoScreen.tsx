...
export function ReportPhotoScreen() {
  ...
  return <ReportPhotoView entryPoint={entryPoint} captureStatus={status} isCameraSupported={captureService.isCameraSupported} isFileSelectionSupported={captureService.isFileSelectionSupported} photo={photo} errorMessage={errorMessage} onBack={() => clearAndBack()} onCapture={() => void capture("capture")} onSelectFile={() => void capture("select")} onRetry={() => void capture("select")} onReview={() => void handleReview()} onExport={() => void handleExport()} onExportDocuments={handleExportDocuments} onDiscard={() => { setPhoto(null); setErrorMessage(null); setStatus("READY"); }} onCaseTab={() => clearAndBack("/case/nearby-agencies?autoLocate=1" as Href)} onGuideTab={() => clearAndBack("/case/guides" as Href)} onDocumentsTab={() => clearAndBack("/case/documents" as Href)} />;
}
