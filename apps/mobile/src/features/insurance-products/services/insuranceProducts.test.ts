...
test("S17 mock exposes three active demo products", async () => { const result = await createMockInsuranceProductsService().listActiveProducts(); assert.equal(result.products.length, 3); assert.deepEqual(result.products.map((product) => product.insurerName), ["TravelCare", "SafeTrip", "GlobeGuard"]); assert.equal(result.products.some((product) => product.status === "INACTIVE"), false); });
